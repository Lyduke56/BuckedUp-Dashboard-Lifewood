...
export function RecentActivityWidget({ products }: RecentActivityWidgetProps) {
  // Grab the 4 most recently "Accepted" or "Published" products (mocking recency by rank)
          ...
          {recent.length === 0 ? (
            ...
            recent.map((product) => (
              ...
              >
                ...
                <div className="text-[10px] font-semibold text-[var(--castleton)] bg-[var(--castleton-glow)] px-2.5 py-1 rounded-full border border-[var(--glass-border)] flex-shrink-0">
                  Approved
                </div>
              </div>
            ))
