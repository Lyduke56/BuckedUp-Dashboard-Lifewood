...
// Pipeline-management tools, shared by admin AND super-admin — the 5-stage
// pipeline refactor expanded super-admin from governance-only to a full
// super-user (products/catalog RLS now allows admin OR super-admin, the
// update-permissions trigger passes both through, and the dashboard UI
// shows super-admin the same review/catalog/edit controls as admin). Most require
...
// super-admin-only in the same refactor, so it lives in super-admin.ts now.)
    ...
    delete_product: tool({
      ...
      execute: ({ rank, id }) =>
        safe(async () => {
          ...
          // so a admin's own session can't even read another user's rows
...
// Same role-gate-returns-{} pattern as the operator/super-admin builders. Super-Admin
// is included since the pipeline refactor made super-admin a full super-user
// (products/catalog RLS, the update-permissions trigger, and the dashboard
// UI's review/catalog/edit controls all treat admin and super-admin identically
// now) — the DB would allow it anyway; this just stops Bucky from
// artificially refusing what the UI already offers.
export function createBuckyAdminActionTools(
  supabase: SupabaseServerClient,
  role: UserRole,
  userId: string,
): ReturnType<typeof buildLeadActionTools> {
  if (role !== "admin") return {} as ReturnType<typeof buildLeadActionTools>;
