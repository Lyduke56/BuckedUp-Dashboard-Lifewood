...
const ROLE_SCHEMA = z.enum(["operator", "admin"]);

// Super-Admin-exclusive actions: account management plus the production plan.
// (The 5-stage pipeline refactor moved production-plan write access from
// admin to super-admin — RLS policies "Super-Admin insert/update/delete" on
// production_plans, and the Planning tab is super-admin-only in the UI — so
// update_production_plan lives here now, not in admin.ts. Super-Admin's
// pipeline/catalog powers come from the shared builder in admin.ts, which
...
// the action; execute() below only actually runs once the super-admin has
// clicked confirm in the chat UI. (Role-gating for these lives in
// createBuckySuperAdminActionTools below, which wraps this builder.)
//
// create_user/delete_user deliberately call the exact same Route Handlers
// the "Manage Users" super-admin UI uses (forwarding the caller's session
...
// self-delete/last-super-admin guards) and duplicating them would risk the two
// copies drifting apart. change_role has no dedicated route (the UI does
// a plain client-side update relying on RLS's super-admin-only policy), so it
...
function buildAdminActionTools(supabase: SupabaseServerClient, request: Request) {
  ...
  return {
    create_user: tool({
      description:
        "Create a new user account by inviting them via email, with the given role. Requires super-admin confirmation before it actually runs.",
      inputSchema: z.object({
        email: z.string().describe("The new account's email address."),
        role: ROLE_SCHEMA.describe("operator or admin"),
      }),
      execute: ({ email, role }) =>
        safe(async () => {
          const res = await fetch(`${origin}/api/super-admin/create-user`, {
            method: "POST",
    ...
    delete_user: tool({
      description: "Delete a user account by email. Requires super-admin confirmation before it actually runs.",
      inputSchema: z.object({ email: z.string().describe("The account's email address.") }),
      execute: ({ email }) =>
        safe(async () => {
          const found = await findUserIdByEmail(email);
          if ("error" in found) return found;
          const res = await fetch(`${origin}/api/super-admin/users/${found.id}`, {
            method: "DELETE",
            headers: { cookie },
          });
          if (!res.ok) {
    ...
    change_role: tool({
      description: "Change an existing user's role by email. Requires super-admin confirmation before it actually runs.",
      inputSchema: z.object({
...
// Bucky is reachable by admin/operator too (see route.ts), but the tools
// above stay super-admin-exclusive — account governance and the production plan
// (super-admin-owned since the pipeline refactor). Returning {} for non-super-admins means the model is never
...
// runtime value genuinely has no keys for non-super-admins, which is what the
...
export function createBuckySuperAdminActionTools(
  supabase: SupabaseServerClient,
  request: Request,
  role: UserRole,
): ReturnType<typeof buildAdminActionTools> {
  if (role !== "super-admin") return {} as ReturnType<typeof buildAdminActionTools>;
